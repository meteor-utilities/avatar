...
***Note that `Avatar.setOptions()` must be called in both server and client code.***
  ...
  - `backgroundColorScheme`: Assign your own scheme for background colors for initials by passing a function here. The color scheme function should take only one argument, the `user` instance and return a CSS compatible color code e.g. "#123abc".
...
- To use a custom coloring scheme for the initials, first write a function that implements your coloring scheme, then assign that to the `backgroundColorScheme` option:
```javascript
...
Avatar.setOptions({
  backgroundColorScheme: customColorScheme,
});
```
