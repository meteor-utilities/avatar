...
- To enable different background colors for initials:
```javascript
Avatar.setOptions({
...
- To use a custom coloring scheme for the initials, first write a function that implments your coloring shceme, then assing that to the `backgroundColorScheme` option:
```javascript
